...
function updateSizesAndScales(current_position, max_rank, duration) {
	...
	var margin_left = Math.max(max_horse_height/2, 35) + state.margin_left + (state.value_type == "ranks" ? 0 : (state.y_axis_format.suffix.length + state.y_axis_format.prefix.length) * (state.y_axis_label_size * 0.5));
	...
	else if (state.height_mode == "flexible" || (state.height_mode == "Fill space" && plot_height > layout.getDefaultPrimaryHeight())) {
